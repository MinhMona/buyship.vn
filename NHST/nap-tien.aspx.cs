    ...
    public partial class nap_tien : System.Web.UI.Page
    {
        ...
        protected void btnSend_Click(object sender, EventArgs e)
        {
            ...
            if (u != null)
            {
                ...
                string kq = AdminSendUserWalletController.Insert_IMG(u.ID, u.Username, Convert.ToDouble(pAmount.Value), 1, Convert.ToInt32(ddlBank.SelectedValue), txtNote.Text, currentDate, username,link);
                if (kq.ToInt(0) > 0)
                {
                    ...
                    if (setNoti != null)
                    {
                        if (setNoti.IsSentNotiAdmin == true)
                        {
                            ...
                            if (managers.Count > 0)
                            {
                                foreach (var manager in managers)
                                {
                                    ...
                                    2, currentDate, u.Username, false);
                                    string strPathAndQuery = Request.Url.PathAndQuery;
                                    string strUrl = Request.Url.AbsoluteUri.Replace(strPathAndQuery, "/");
                                    string datalink = "" + strUrl + "manager/HistorySendWallet/";
                                ...
                                foreach (var admin in admins)
                                {
                                    try
                                    {
                                        PJUtils.SendMailGmail_new( admin.Email,
                                            "Thông báo tại BUYSHIP.", "Có yêu cầu nạp tiền mới từ tài khoản " + u.Username + " với số tiền " + Convert.ToDouble(pAmount.Value) + "", "");
                                    }
                                    catch { }
                                }
                            }
                        ...
                        if (setNoti.IsSendEmailUser == true)
                        {
                            try
                            {
                                ...
                                html.Append("<tr>");
                                html.Append("<td style=\"padding: 10px; background-color: #fca777; color: #fff; text-align: center\"><strong><p>KÍNH CHÀO QUÝ KHÁCH!</p><p><a style=\"text-decoration: none\" href=\"https://FALOSHIP.COM/\" target=\"_blank\"><strong>FALOSHIP.COM<strong></a> THÔNG BÁO NẠP VÍ THÀNH CÔNG</p></strong></td>");
                                html.Append("</tr>");
                                html.Append("<tr>");
                                ...
                                html.Append("</tr>");
                                html.Append("<tr>");
                                html.Append("<td><a style=\"text-decoration: none\" href=\"https://FALOSHIP.COM/\" target=\"_blank\"><strong>FALOSHIP.COM<strong></a> chân thành cảm ơn!</td>");
                                html.Append("</tr>");
                                html.Append("<tr>");
                                html.Append("<td>==============================</td>");
                                html.Append("<tr>");
                                ...
                                //"FALOSHIP.COM THÔNG BÁO NẠP VÍ THÀNH CÔNG",
                                ...
                                PJUtils.SendMailGmail_new( u.Email,
                                                        "FALOSHIP.COM THÔNG BÁO GỬI YÊU CẦU NẠP VÍ THÀNH CÔNG",
