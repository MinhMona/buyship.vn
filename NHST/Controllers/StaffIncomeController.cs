...
namespace NHST.Controllers
{
    public class StaffIncomeController
    {
        ...
        {
            var sql = @"SELECT st.ID,st.MainOrderID,st.OrderTotalPrice,st.PercentReceive,st.UID,st.Username,st.RoleID,st.Status,st.TotalPriceReceive,st.CreatedDate,mo.CompleteDate ";
            sql += "from tbl_StaffIncome st ";
            sql += " left outer join(select * from tbl_MainOder) as mo ON st.MainOrderID = mo.ID ";
            ...
            while (reader.Read())
            {
                ...
                if (reader["CompleteDate"] != DBNull.Value)
                    entity.CompleteDate = Convert.ToDateTime(reader["CompleteDate"].ToString()).ToString("dd/MM/yyyy HH:mm:ss");
