...
namespace NHST.Controllers
{
    public class SmallPackageController
    {
        ...
        public static string UpdateDateScanKho(int ID, DateTime DateSanKho)
        {
            using (var dbe = new NHSTEntities())
            {
                var a = dbe.tbl_SmallPackage.Where(x => x.ID == ID).FirstOrDefault();
    ...
    int Status, string Description, DateTime ModifiedDate, string ModifiedBy)
        ...
        public static List<tbl_SmallPackage> GetListOrderTransactionCodeKhoVN(string OrderTransactionCode)
        {
            using (var dbe = new NHSTEntities())
            {
                List<tbl_SmallPackage> lmls = new List<tbl_SmallPackage>();
        ...
        public static List<tbl_SmallPackage> GetListOrderTransactionCodeKhoTQ(string OrderTransactionCode)
        {
            using (var dbe = new NHSTEntities())
            {
                List<tbl_SmallPackage> lmls = new List<tbl_SmallPackage>();
                lmls = dbe.tbl_SmallPackage.Where(ad => ad.OrderTransactionCode == OrderTransactionCode && ad.Status < 3).ToList();
                return lmls;
            }
        }
        ...
        public static List<ShowBigPackage> GetBuyBigPackageBySQL_DK(int BigPackageID, string s, int pageIndex, int pageSize)
        {
            ...
            sql += "when 1 then N'<span class=\"white-text badge blue darken-2\">Chưa về kho JP</span>' ";
            sql += "when 2 then N'<span class=\"white-text badge orange darken-2\">Đã về kho JP</span>' ";
        ...
        public static double GetTotalWeight(string PriceType, string fd, string td)
        {
            var sql = @"select total=SUM(CAST(" + PriceType + " as float)) ";
               sql += " FROM dbo.tbl_SmallPackage ";
               sql += " Where Status <> 0 ";
            if (!string.IsNullOrEmpty(fd))
            {
                var df = DateTime.ParseExact(fd, "dd/MM/yyyy HH:mm", null);
                sql += " AND DateSanKho >= CONVERT(VARCHAR(24),'" + df + "',113) ";
            }
            if (!string.IsNullOrEmpty(td))
            {
                var dt = DateTime.ParseExact(td, "dd/MM/yyyy HH:mm", null);
                sql += " AND DateSanKho <= CONVERT(VARCHAR(24),'" + dt + "',113) ";
            }
            var reader = (IDataReader)SqlHelper.ExecuteDataReader(sql);
            double total = 0;
            while (reader.Read())
   ...
   int Status, string Description, DateTime ModifiedDate, string ModifiedBy, int MainOrderCodeID)
        ...
        public static string UpdateStatusAndIsLostAndDateInKhoDich_NonDate(int ID, int Status, bool IsLost
        ...
        public static string Update_Thang(int ID, int BigPackageID, string OrderTransactionCode, string ProductType, double FeeShip, double Weight, double Volume,
          int Status, string Description, DateTime ModifiedDate, string ModifiedBy, DateTime DateInTQWarehouse)
        {
            using (var dbe = new NHSTEntities())
            {
                tbl_SmallPackage a = dbe.tbl_SmallPackage.Where(ad => ad.ID == ID).FirstOrDefault();
        ...
        public static List<tbl_SmallPackage> GetAllByBigPackageID(int BigPackageID)
        {
            using (var dbe = new NHSTEntities())
            {
                List<tbl_SmallPackage> ps = new List<tbl_SmallPackage>();
