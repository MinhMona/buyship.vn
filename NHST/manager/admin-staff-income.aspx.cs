...
namespace NHST.manager
{
    public partial class admin_staff_income : System.Web.UI.Page
    {
        ...
        protected void btnExcel_Click(object sender, EventArgs e)
        {
            ...
            StrExport.Append("  <tr>");
            StrExport.Append("      <th tyle=\"mso-number-format:'\\@'\"><strong>Mã đơn hàng</strong></th>");
            StrExport.Append("      <th tyle=\"mso-number-format:'\\@'\"><strong>Phần trăm</strong></th>");
            StrExport.Append("      <th tyle=\"mso-number-format:'\\@'\"><strong>Hoa hồng</strong></th>");
            StrExport.Append("      <th tyle=\"mso-number-format:'\\@'\"><strong>Username</strong></th>");
            StrExport.Append("      <th tyle=\"mso-number-format:'\\@'\"><strong>Quyền hạn</strong></th>");
            StrExport.Append("      <th tyle=\"mso-number-format:'\\@'\"><strong>Trạng thái</strong></th>");
            StrExport.Append("      <th tyle=\"mso-number-format:'\\@'\"><strong>Ngày hoàn thành</strong></th>");
            StrExport.Append("  </tr>");
            foreach (var item in la)
            {
                StrExport.Append("  <tr>");
                StrExport.Append("      <td>" + item.MainOrderID + "</td>");
                StrExport.Append("      <td>" + item.Percent + " %</td>");
                StrExport.Append("      <td>" + string.Format("{0:N0}", Convert.ToDouble(item.TotalPriceReceive)) + "</td>");
