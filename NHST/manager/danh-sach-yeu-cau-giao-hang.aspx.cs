    ...
    public partial class danh_sach_yeu_cau_giao_hang : System.Web.UI.Page
    {
        ...
        public void pagingall(List<tbl_YCG> acs, int total)
        {
            ...
            if (total > 0)
            {
                ...
                var list = HttpContext.Current.Session["ListID"] as List<ListID>;
                for (int i = 0; i < acs.Count; i++)
                {
                    ...
                    if (item.Status == 1)
                    {
                        ...
                        if (list != null)
                        {
                            var check = list.Where(x => x.ID == item.ID).SingleOrDefault();
                            if (check != null)
                            {
                    ...
                    hcm.Append("<td>" + item.StaffNote + "</td>");
                    hcm.Append("<td>" + PJUtils.ShowStatusGiaoHang(Convert.ToInt32(item.Status)) + "</td>");
            ...
            if (ac != null)
            {
                if (ID > 0)
                {
                    ...
                    if (yc != null)
                    {
                        ...
                        {
                            PJUtils.ShowMessageBoxSwAlert("Đơn hàng đã được duyệt", "i", true, Page);
                        }
                        YCGController.UpdateStaffNote(yc.ID, txtStoreNote.Text, username_current, currentDate);
                    }
                }
            }
        }
        ...
        protected void btnUpdateStaff_Click(object sender, EventArgs e)
        {
            ...
            if (obj_user != null)
            {
                ...
                {
                    int status = hdfID.Value.ToInt(0);
                    List<ListID> list = new List<ListID>();
                    if (HttpContext.Current.Session["ListID"] != null)
                    {
                        list = (List<ListID>)HttpContext.Current.Session["ListID"];
                    }
                    if (list.Count > 0)
                    {
                        foreach (var item in list)
                        {
                            var yc = YCGController.GetByID(item.ID);
                            if (yc != null)
                            {
                                if (yc.Status != 2)
                                {                                    
                                    YCGController.Update(yc.ID, status, username, currentDate);                                    
                                }
                                YCGController.UpdateStaffNote(yc.ID, txtStoreNote.Text, username, currentDate);
                            }
                        }
                        Session["ListID"] = null;
                        PJUtils.ShowMessageBoxSwAlert("Cập nhật thành công.", "s", true, Page);
        ...
        public static string CheckStaff(int MainOrderID)
        {
            ...
            if (list != null)
            {
                if (list.Count > 0)
                {
                    var check = list.Where(x => x.ID == MainOrderID).FirstOrDefault();
                    if (check != null)
                    {
                        list.Remove(check);
                    }
            ...
            else
            {
                ...
                d.ID = MainOrderID;
                ldep.Add(d);
                HttpContext.Current.Session["ListID"] = ldep;
                JavaScriptSerializer serializer = new JavaScriptSerializer();
