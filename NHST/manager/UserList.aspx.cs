...
namespace NHST.manager
{
    public partial class UserList1 : System.Web.UI.Page
    {
        ...
        public void LoadData()
        {
            ...
            var la = AccountController.GetListUserBySQL_list(search, searchid, searchphone, page, 10);
            int total = AccountController.GetTotalUser_List(search, searchid, searchphone);
            pagingall(la, total);
        }
        ...
        public void pagingall(List<View_UserList> acs, int total)
        {
            ...
            if (total > 0)
            {
                ...
                for (int i = 0; i < acs.Count; i++)
                {
                    ...
                    var a = AccountController.GetByID(item.ID);
                    hcm.Append("<tr>");
                    ...
                    if (!string.IsNullOrEmpty(a.SaleID.ToString()))
                    {
                        if (a.SaleID > 0)
                        {
                            hcm.Append("<td>" + AccountController.GetByID(Convert.ToInt32(a.SaleID)).Username + "</td>");
        ...
        protected void btnSearchPhone_Click(object sender, EventArgs e)
        {
            ...
            {
                Response.Redirect("userlist?p=" + searchphone + "");
            }
        }
        protected void btnSearchID_Click(object sender, EventArgs e)
        {
            ...
            {
                Response.Redirect("userlist?id=" + searchid + "");
            }
        }
