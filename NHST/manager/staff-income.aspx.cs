...
namespace NHST.manager
{
    public partial class staff_income : System.Web.UI.Page
    {
        ...
        public void LoadData()
        {
            ...
            var a = AccountController.GetByUsername(Session["userLoginSystem"].ToString());
            double TotalPaid = 0;
            double TotalNotPaid = 0;
            TotalPaid = StaffIncomeController.GetTotalPrice(a.ID, 2, "TotalPriceReceive", search, fd, td);
            TotalNotPaid = StaffIncomeController.GetTotalPrice(a.ID, 1, "TotalPriceReceive", search, fd, td);
            ltrTienDaThanhToan.Text = string.Format("{0:N0}", TotalPaid) + " VNĐ";
            ltrTienChuaThanhToan.Text = string.Format("{0:N0}", TotalNotPaid) + " VNĐ";
            var la = StaffIncomeController.GetAllByUIDWithSQL(a.ID, st.ToInt(0), search, fd, td, page, 20);
            int total = StaffIncomeController.GetTotalWithUID(a.ID, st.ToInt(0), search, fd, td);
            pagingall(la, total);
        }
        protected void search_Click(object sender, EventArgs e)
        {
            ...
            {
                Response.Redirect("staff-income");
            }
            else
            {
                Response.Redirect("staff-income?s=" + searchname + "&fd=" + fd + "&td=" + td + "&st=" + st + "");
            }
        ...
        public void pagingall(List<StaffInCome> acs, int total)
        {
            ...
            if (total > 0)
            {
                ...
                for (int i = 0; i < acs.Count; i++)
                {
                    var item = acs[i];
                    //if (item.Status == 1)
                    //    TotalNotPaid += item.TotalPriceReceive;
                    //if (item.Status == 2)
                    //    TotalPaid += item.TotalPriceReceive;
                    hcm.Append("<tr>");
                    hcm.Append("<td>" + item.MainOrderID + "</td>");
                    hcm.Append("<td>" + item.Percent + "</td>");
                    hcm.Append("<td>" + string.Format("{0:N0}", item.TotalPriceReceive) + " VNĐ</td>");
                    ...
                    hcm.Append("<a href='OrderDetail.aspx?id=" + item.MainOrderID + "' data-position=\"top\">");
                    hcm.Append(" <i class=\"material-icons valign-center\">remove_red_eye</i><span>Xem chi tiết đơn</span></a>");
                    hcm.Append("</div>");
